...
data_wrangling = function(option_list)
{
  ...
  cat("\n")
  cat(sprintf(as.character(names(summary(as.factor(MPRA_Real_tile$Label_2))))))
  cat("\n")
  cat(sprintf(as.character(summary(as.factor(MPRA_Real_tile$Label_2)))))
  cat("\n")
  
  
  
  
  check_1<-MPRA_Real_tile[which(MPRA_Real_tile$VAR%in%Sankaran_MPRA_Sig_subset_wide$VAR),]
  ...
  cat("\n")
  cat(str(unique(check_1$carried_variants)))
  cat("\n")
  cat(str(unique(check_1$REAL_TILE_Plus_carried_variants)))
  cat("\n")
  cat(sprintf(as.character(names(summary(as.factor(check_1$Label_2))))))
  cat("\n")
  cat(sprintf(as.character(summary(as.factor(check_1$Label_2)))))
  cat("\n")
