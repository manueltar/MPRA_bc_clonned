...
cummulative_CLASSIF_with_ctrls = function(option_list)
{
  ...
  for(i in 1:length(array_Elements))
  {
    ...
    #(array_Elements_sel == "Element_87;NCGR")
    
    if(array_Elements_sel == 'Element_999;16_265671_A_G')
    {
  ...
  cat("\n")
  cat(str(unique(ALL_CT_df$carried_variants)))
  cat("\n")
  cat(str(unique(ALL_CT_df$KEY_Plus_carried_variants)))
  cat("\n")
...
cummulative_CLASSIF = function(option_list)
{
  ...
  cat("\n")
  cat(str(unique(KEY_collapse_EXCLUDED_other_labels$VAR)))
  cat("\n")
  cat(str(unique(KEY_collapse_EXCLUDED_other_labels$carried_variants)))
  cat("\n")
  cat(str(unique(KEY_collapse_EXCLUDED_other_labels$KEY_Plus_carried_variants)))
  cat("\n")
...
cummulative_PLOTS_enhancer = function(option_list)
{
  ...
  ALL_CT_df_subset<-unique(ALL_CT_df[,-which(colnames(ALL_CT_df) == "VEP_DEF_LABELS")])
  
  cat("ALL_CT_df_subset_0\n")
  cat(str(ALL_CT_df_subset))
  cat("\n")
  cat(str(unique(ALL_CT_df_subset$VAR)))
  cat("\n")
  ...
  Total_N_PRE<-unique(ALL_CT_df_subset[,c(which(colnames(ALL_CT_df_subset) == "Cell_Type"),
                                      which(colnames(ALL_CT_df_subset) == "KEY_Plus_carried_variants"))])
  
  cat("Total_N_PRE_0\n")
  cat(str(Total_N_PRE))
  cat("\n")
  
  Total_N_PRE.dt<-data.table(Total_N_PRE, key="Cell_Type")
  
  
  Total_N_POST<-as.data.frame(Total_N_PRE.dt[,(TOTAL=.N), by=key(Total_N_PRE.dt)], stringsAsFactors=F)
  
  colnames(Total_N_POST)[which(colnames(Total_N_POST) == "V1")]<-"TOTAL"
  
  cat("Total_N_POST_0\n")
  cat(str(Total_N_POST))
  cat("\n")
  
  
  ##### Subset-----
  
  subset<-unique(ALL_CT_df_subset[which(ALL_CT_df_subset$enhancer_CLASS_TILES >0),])
  ...
  TABLE_enhancer<-merge(TABLE_enhancer,
                        Total_N_POST,
                        by="Cell_Type")
  
  
  cat("TABLE_enhancer_2\n")
  cat(str(TABLE_enhancer))
  cat("\n")
...
cummulative_PLOTS_ASE = function(option_list)
{
  ...
  subset<-unique(ALL_CT_df_subset[which(ALL_CT_df_subset$ASE_CLASS_TILES >0),])
...
cummulative_PLOTS_E_Plus_ASE = function(option_list)
{
  ...
  subset<-unique(ALL_CT_df_subset[which(ALL_CT_df_subset$E_Plus_ASE_CLASS_TILES >0),])
  ...
  TABLE_E_Plus_ASE<-merge(TABLE_E_Plus_ASE,
                   Total_N_POST,
                   by="Cell_Type")
  
  
  cat("TABLE_E_Plus_ASE_2\n")
  cat(str(TABLE_E_Plus_ASE))
  cat("\n")
