...
Reference_fasta_printer = function(option_list)
{
  ...
  #### READ and transform selected_option ----
  
  selected_option = opt$selected_option
  
  cat("selected_option_\n")
  cat(sprintf(as.character(selected_option)))
  cat("\n")
...
for(i in 1:length(levels(REP$parameters)))
{
  ...
  BC_table_subset<-BC_table[which(BC_table$subsampling == "100_percent" &
                                    BC_table$parameters ==  selected_option),]
  
  BC_table_subset<-droplevels(BC_table_subset)
  ...
  cat(sprintf(as.character(names(summary(BC_table_subset$parameters)))))
  cat("\n")
  cat(sprintf(as.character(summary(BC_table_subset$parameters))))
  cat("\n")
  
  cat(sprintf(as.character(names(summary(BC_table_subset$pool)))))
  cat("\n")
  cat(sprintf(as.character(summary(BC_table_subset$pool))))
  cat("\n")
  # ##HERE HERE
  # quit(status = 1)
      ...
      list_FASTA_bc<-list()
        ...
        for(h in 1:dim(BC_table_subset_sel)[1])
        {
          ...
          a.df<-as.data.frame(cbind(rep(MPRA_Rosetta_subset_sel$chr, length_barcode_string),
                              ...
                              rep(as.character(bc_sel$pool), length_barcode_string),
                              barcode_string), stringsAsFactors=F)
          
          colnames(a.df)<-c("chr","start","REAL_TILE","carried_variants","seq_name","pool","Barcode")
  ...
  cat("\n")
  cat(sprintf(as.character(names(summary(as.factor(TABLE_FASTA_ordered$pool))))))
  cat("\n")
  cat(sprintf(as.character(summary(as.factor(TABLE_FASTA_ordered$pool)))))
  cat("\n")
  
  
  check<-unique(TABLE_FASTA_ordered[,c(which(colnames(TABLE_FASTA_ordered) == "seq_name"),which(colnames(TABLE_FASTA_ordered) == "pool"))])
  
  cat("------------------------------------------------------------------------------------------------>check\n")
  str(check)
  cat("\n")
  cat(sprintf(as.character(names(summary(as.factor(check$pool))))))
  cat("\n")
  cat(sprintf(as.character(summary(as.factor(check$pool)))))
  cat("\n")
  cat("\n")
  cat("\n")
  cat("\n")
  ...
  FINAL_table<-TABLE_Q_DEF
  
  # merge(MPRA_Rosetta_RT_format,
  #                    TABLE_Q_DEF,
...
svgname<-paste("Graphs_",type,"_QC_thresholds_",selected_option,".svg", sep='')
makesvg = TRUE
...
svgname<-paste("Graphs_",type,"_FINAL_stacked_barplot_",selected_option,".svg", sep='')
makesvg = TRUE
...
main = function() {
  ...
  option_list <- list(
    ...
    make_option(c("--QC_dnaALT_REF_threshold"), type="character", default=NULL, 
                metavar="type1", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
    make_option(c("--selected_option"), type="character", default=NULL, 
                metavar="type", 
                help="Path to tab-separated input file listing regions to analyze. Required."),
