...
dot_plot_graphs = function(option_list)
{
  ...
  LONG_MATRIX$carried_variants<-gsub("chr","",LONG_MATRIX$carried_variants)
  
  
  LONG_MATRIX$TILE<-LONG_MATRIX$Tile
  ...
  indx.int<-c(which(colnames(LONG_MATRIX) == "REAL_TILE_Plus_carried_variants"),which(colnames(LONG_MATRIX) == "VAR"),which(colnames(LONG_MATRIX) == "carried_variants"),
              ...
              which(colnames(LONG_MATRIX) == "end"))
  ...
  colnames(LONG_MATRIX_subset)<-c("REAL_TILE_Plus_carried_variants","VAR","carried_variants","TILE","chr","start","end")
...
# quit(status = 1)
  ...
  for(i in 1:length(VARS))
  {
    ...
    if(VAR_sel == "chr10_103835537_C_T")
    {
    ...
    for(k in 1:length(carried_variants_array))
    {
      ...
      if(carried_variants_sel != "NCGR")
      {
        ...
        max_POS<-max(LONG_MATRIX_subset_sel$end)
