...
data_wrangling_FC_ASE = function(option_list)
{
  ...
  for(i in 1:length(VARS))
  {
    ...
    if(VAR_sel == 'chr10_103872328_A_T')
    {
    ...
    if(dim(MPRA_Real_tile_sel)[1] > 0)
    {
      ...
      for(l in 1:length(Cell_Types_array))
      {
        ...
        MPRA_Real_tile_sel_CT_sel<-unique(MPRA_Real_tile_sel[which(MPRA_Real_tile_sel$Cell_Type == Cell_Types_array_sel),-which(colnames(MPRA_Real_tile_sel) == 'VEP_DEF_LABELS')])
        
        if(Condition_DEBUG == 1)
        {
          cat("MPRA_Real_tile_sel_CT_sel_0\n")
          cat(str(MPRA_Real_tile_sel_CT_sel))
          cat("\n")
          cat(str(unique(MPRA_Real_tile_sel_CT_sel$Cell_Type)))
          cat("\n")
        }
        ...
        if(dim(MPRA_Real_tile_sel_CT_sel)[1] > 0)
        {
          ...
          check_DUP<-which(duplicated(MPRA_Real_tile_LogFC_ASE))
          ...
          if(Condition_DEBUG == 1)
          {
            cat("MPRA_Real_tile_LogFC_ASE_0\n")
            cat(str(MPRA_Real_tile_LogFC_ASE))
            cat("\n")
            cat(str(unique(MPRA_Real_tile_LogFC_ASE$variable)))
            cat("\n")
          }
          
          MPRA_Real_tile_LogFC_ASE<-unique(MPRA_Real_tile_LogFC_ASE)
          
          if(Condition_DEBUG == 1)
          {
            cat("MPRA_Real_tile_LogFC_ASE_1\n")
            cat(str(MPRA_Real_tile_LogFC_ASE))
            cat("\n")
            cat(str(unique(MPRA_Real_tile_LogFC_ASE$variable)))
            cat("\n")
          ...
          if(Condition_DEBUG == 1)
          {
            cat("MPRA_Real_tile_LogFC_ASE_wide_1\n")
            cat(str(MPRA_Real_tile_LogFC_ASE_wide))
            cat("\n")
          }
          ...
          #   setwd(out2)
          #   
          #   cat("Printing:MPRA_Real_tile_LogFC_ASE\n")
          #   write.table(MPRA_Real_tile_LogFC_ASE,file="Duplicates_wide.tsv",sep="\t",quote=F, row.names = F)
          #   
          #   #########################################
          #   quit(status = 1)
          # }
