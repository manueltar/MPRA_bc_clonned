...
cummulative_CT_Label2_and_Log_Rank_test_enhancer = function(option_list)
{
  ...
  cat("\n")
  cat(sprintf(as.character(names(summary(CUMMULATIVE_CLASSES$Fig1_Annot_Category)))))
  cat("\n")
  cat(sprintf(as.character(summary(CUMMULATIVE_CLASSES$Fig1_Annot_Category))))
  cat("\n")
  ...
  RESCUE_NCGR<-CUMMULATIVE_CLASSES[which(CUMMULATIVE_CLASSES$Fig1_Annot_Category == "NCGR"),]
  ...
  cat("\n")
  cat(sprintf(as.character(names(summary(VAR_Prioritization_dB_subset$Fig1_Annot_Category)))))
  cat("\n")
  cat(sprintf(as.character(summary(VAR_Prioritization_dB_subset$Fig1_Annot_Category))))
  cat("\n")
  
  #### Merge with VAR_Prioritization_dB ----
  
  CUMMULATIVE_CLASSES_restricted<-merge(CUMMULATIVE_CLASSES_restricted,
                                        VAR_Prioritization_dB_subset,
                                        by="VAR")
  ...
  if(Condition_DEBUG == 1)
  {
    ...
    cat(str(unique(CUMMULATIVE_CLASSES_restricted$KEY_Plus_carried_variants)))
    cat("\n")
    cat(sprintf(as.character(names(summary(CUMMULATIVE_CLASSES_restricted$Fig1_Annot_Category)))))
    cat("\n")
    cat(sprintf(as.character(summary(CUMMULATIVE_CLASSES_restricted$Fig1_Annot_Category))))
    cat("\n")
    cat(sprintf(as.character(names(summary(CUMMULATIVE_CLASSES_restricted$Fig1_Annot_Category)))))
    cat("\n")
    cat(sprintf(as.character(summary(CUMMULATIVE_CLASSES_restricted$Fig1_Annot_Category))))
  ...
  CUMMULATIVE_CLASSES_restricted_subset<-unique(CUMMULATIVE_CLASSES_restricted[,c(which(colnames(CUMMULATIVE_CLASSES_restricted) == "VAR"),
                                                                                ...
                                                                                which(colnames(CUMMULATIVE_CLASSES_restricted) == "Fig1_Annot_Category"),
  ...
  Log_rank_df$Fig1_Annot_Category<-Log_rank_df$Fig1_Annot_Category
  ...
  CUMMULATIVE_CLASSES_restricted.dt<-data.table(CUMMULATIVE_CLASSES_restricted, key=c("Cell_Type","Fig1_Annot_Category"))
  ...
  Freq_TOTAL.dt<-data.table(Freq_TOTAL, key=c("Fig1_Annot_Category"))
  ...
  Log_rank_df.dt<-data.table(Log_rank_df, key=c("Cell_Type","Fig1_Annot_Category","TILE"))
  ...
  Freq_table<-Freq_table[order(Freq_table$Fig1_Annot_Category,Freq_table$Cell_Type, Freq_table$TILE),]
  ...
  for(i in 1:length(CT_levels))
  {
    ...
    Fig1_Annot_Category_labels<-levels(Log_rank_df_sel$Fig1_Annot_Category)
    
    for(k in 1 :length(Fig1_Annot_Category_labels))
    {
      BAIT<-Fig1_Annot_Category_labels[k]
      ...
      for(h in 1 :length(Fig1_Annot_Category_labels))
      {
        
        PREY<-Fig1_Annot_Category_labels[h]
        ...
        Log_rank_df_sel_subset<-Log_rank_df_sel[which(Log_rank_df_sel$Fig1_Annot_Category%in%Mini_comparison),]
        ...
        FLAG_diversity<-as.integer(length(unique(Log_rank_df_sel_subset$Fig1_Annot_Category)))
        ...
        if(FLAG_diversity >1)
        {
          ...
          if(FLAG_diversity_2 >1)
          {
            ...
            STATS_test<-logrank.test(
              ...
              Log_rank_df_sel_subset$Fig1_Annot_Category,
            ...
            colnames(a.df)<-c("Fig1_Annot_Category_c1","Fig1_Annot_Category_c2","minus_logpval")
      ...
      }# h in 1 :length(unique(Log_rank_df_sel$Fig1_Annot_Category)
    ...
    }#k in 1 :length(unique(Log_rank_df_sel$Fig1_Annot_Category))
    ...
    vector_colors_Fig1_Annot_Category<-c("greenyellow","blue","dark cyan","black","red")
    ...
    path5<-paste(out2,'CT_Fig1_Annot_Category_CUMULATIVE_CURVES','/', sep='')
    ...
    indx_Fig1_Annot_Category<-which(colnames(Freq_table_sel) == "Fig1_Annot_Category")
    indx.yaxis<-which(colnames(Freq_table_sel) == "Perc")
    mycols <- vector_colors_Fig1_Annot_Category
    ...
    lab <- as.character(unique(Freq_table_sel[,indx_Fig1_Annot_Category]))
    ...
    for (iteration_graph in 1:length(lab))  {
      ...
      ind <- which(Freq_table_sel[,indx_Fig1_Annot_Category]==lab[iteration_graph])
  ...
  if(length(list_ABSOLUTE_DEF) >0)
  {
    ...
    write.table(FINAL_df_ABSOLUTE, file="enhancer_Fig1_Annot_Category_log_Rank_test_STAT.tsv", sep="\t", row.names = F, quote = F)
  ...
  if(length(list_ABSOLUTE_DEF_Freq) >0)
  {
    ...
    write.table(FINAL_ABSOLUTE_DEF_Freq, file="enhancer_Fig1_Annot_Category_Cummulative_Freq_table.tsv", sep="\t", quote=F,row.names = F)
...
cummulative_CT_Label2_and_Log_Rank_test_E_Plus_ASE = function(option_list)
{
  ...
  if(length(list_ABSOLUTE_DEF) >0)
  {
    ...
    write.table(FINAL_df_ABSOLUTE, file="E_Plus_ASE_Fig1_Annot_Category_log_Rank_test_STAT.tsv", sep="\t", row.names = F, quote = F)
  ...
  if(length(list_ABSOLUTE_DEF_Freq) >0)
  {
    ...
    write.table(FINAL_ABSOLUTE_DEF_Freq, file="E_Plus_ASE_Fig1_Annot_Category_Cummulative_Freq_table.tsv", sep="\t", quote=F,row.names = F)
